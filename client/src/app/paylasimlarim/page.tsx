...
import { ProfilePagination } from "@/app/paylasimlarim/_components/MyExperiencesPagination";
...
export default async function MyExperiencesPage({
  searchParams,
}: MyExperiencesPageProps) {
  const session = await getServerSession();

  if (!session?.user) {
    redirect("/giris?redirect=/paylasimlarim");
  }
  ...
  if (pagination.totalPages > 0 && page > pagination.totalPages) {
    const redirectUrl =
      pagination.totalPages === 1
        ? "/paylasimlarim"
        : `/paylasimlarim?page=${pagination.totalPages}`;
